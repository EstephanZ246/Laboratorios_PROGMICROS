...
;MACROS
    ...
    MOVLW 0XF8 //248.25 Colocamos valor obtenido con formula para tener 2ms=4*(1/31000)*(256-TMR0)*(2)
...
REINICIAR_TMR1 MACRO
    BANKSEL TMR1L
    /////// VALOR ES 64567.25 EN HEX ES: FC37 PARA TENER 1 SEG
    MOVLW 0X37 // VALOR PARA BYTE INFERIOR TIMER 1
    MOVWF TMR1L //CARGAR A BYTE INFERIOR
    MOVLW 0XFC//VALOR BYTE SUPERIOR
...
//////////////////////////////////////////////////////////////////////////
    ...
    MOVLW 0X01
    XORWF MUXEO // APLICAMOS XOR PARA QUE PODAMOS HACER EL MULTIPLEXADO
...
INTERRUPCION_TIMER2:
    ...
    XORWF ENABLE_DISPLAYS // ENCENDER Y APAGAR LOS DISPLAYS
    REINICIAR_TMR2
...
CONFIGURAR_TMMR2:
    BANKSEL T2CON
    BCF T2CON,1///PRESCALER
    BSF T2CON,0///PRESCALER  // 1:4 
    BSF T2CON,6///POSTCALER
    BCF T2CON,5///POSTCALER // 1:10 
    BCF T2CON,4///POSTCALER
    BSF T2CON,3///POSTCALER
    BSF T2CON,2 // TIMER 2 ON
    MOVLW 0x2F//0X30//48.4375 250ms
    MOVWF PR2
    REINICIAR_TMR2
    RETURN
...
LOOP:
    ...
    CLRF PORTE // LIMPIARMOS PUERTOS
    CLRF PORTC // LIMPIAMOS PUERTOS
    BTFSC MUXEO,0 // VARIFICAMOS VALOR DE MUXEO PARA ENCENDER Y CARGAR VALOR CORRESPONDIENTE A CADA DISPLAY
    CALL DISPLAY1
    BTFSS MUXEO,0
    CALL DISPLAY2
    GOTO LOOP 
...
DISPLAY1:
    CLRF PORTE // LIMPIARMOS PUERTOS
    CLRF PORTC
    BSF PORTE,0  // ENCENDEMOS DISPLAY CORRESPONDIENTE
    BCF PORTE,1
    MOVF DECENA,W // MOVEMOS VALOR OBTENIDO DE DECENA PARA IMPRIMIRLO 
...
DISPLAY2:  
    CLRF PORTE // LIMPIARMOS PUERTOS
    CLRF PORTC
    BSF PORTE,1  // ENCENDEMOS DISPLAY CORRESPONDIENTE
    BCF PORTE,0
    MOVF UNIDAD,W// MOVEMOS VALOR OBTENIDO DE UNIDAD PARA IMPRIMIRLO 
    CALL TABLAS   // OBTENEMOS VALOR CORRESPONDIENTE PARA DISPLAY
    MOVWF PORTC  // IMPRIMIMOS EN EL PUERTO
    RETURN
END
